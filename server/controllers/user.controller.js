...
export const updateUser = async (req, res, next) => {
  ...
  if (password) {
    if (password.length < 6) {
      return next(
        errorHandler(400, "The length of the password must be greater than 6")
      );
    }
  ...
  if (username) {
    ...
    if (!username.match(/^[a-zA-Z0-9]+$/)) {
      return next(
        errorHandler(400, "Username can only containe letters and numbers")
      );
    }
...
export const deleteUser = async (req, res, next) => {
  ...
  if (user.id !== id) {
    return next(errorHandler(404, "You are not authorized to delete this"));
  }
...
export const signOut = async (req, res, next) => {
  try {
    res.clearCookie("access_token").status(200).json("Signing out successful");
  } catch (err) {
    next(err);
  }
};
